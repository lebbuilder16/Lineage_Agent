...
async def cleanup_live_token_sol_flows(db) -> int:
    """Delete sol_flow rows for mints that were never confirmed rugged.

    These rows were generated by the pre-fix code path that called
    trace_sol_flow on live tokens, producing false-positive extraction data
    (fees, rent, launchpad costs falsely attributed as deployer extraction).

    Safe to run repeatedly — idempotent. Returns number of rows deleted.
    """
    cursor = await db.execute(
        """
        DELETE FROM sol_flows
        WHERE mint NOT IN (
            SELECT DISTINCT mint
            FROM intelligence_events
            WHERE event_type = 'token_rugged'
        )
        """
    )
    await db.commit()
    ...
    if deleted:
        logger.info(
            "cleanup_live_token_sol_flows: removed %d false-positive sol_flow rows "
            "(live token traces — fees/rent wrongly attributed as extraction)",
            deleted,
        )
    return deleted
...
async def _maintenance_loop() -> None:
    ...
    # One-time startup cleanup: remove false-positive sol_flow rows for live
    # tokens generated before the rug-gate fix was deployed. Idempotent.
    try:
        if hasattr(_cache_backend, '_get_conn'):
            _startup_db = await _cache_backend._get_conn()
            await cleanup_live_token_sol_flows(_startup_db)
    except Exception:
        logger.debug("Startup sol_flow cleanup skipped (table may not exist yet)")
